...
export default function Order() {
  ...
  const feeCal = Number(
    Big(Number(order.receiveFiatValue)).mul(
      Big(Number(order.fee)).div(Big(100))
    )
  );
  ...
  if (order.discount_percentage) {
    productPrice = `${formatNumber(
      Number(
        Big(Number(order.price))
          .mul(Big(100).minus(Big(Number(order.discount_percentage))))
          .div(100)
      )
    )}`;
  } else {
    productPrice = `${formatNumber(
      Number(Big(Number(order.receiveFiatValue)).plus(Big(Number(feeCal))))
    )}`;
  }

  const rateExchange = Number(
    Big(Number(productPrice)).div(Big(Number(order.payAmount)))
  );
