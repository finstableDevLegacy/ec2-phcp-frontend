import { Dispatch, SetStateAction, useEffect, useState } from "react";
import { Link, useLocation } from "@remix-run/react";
import { MerchantOrder, OrderState, OrderStatus } from "~/type/order";
import Countdown from "~/components/countdown";
import { shorten } from "~/utils/shorten";
import TokenListbox from "~/components/ui/tokenListbox";
import Loading from "~/components/loading";
import { useApproveToken, useOrderStatus } from "~/hooks";
import { Chain, useAccount, useConnect, useNetwork, useSigner } from "wagmi";
import { Transcrypt__factory } from "~/typechain";
import { formatEther, parseUnits } from "ethers/lib/utils";
import ApproveToken from "~/components/approve-token";
import {
  CheckCircleIcon,
  ClockIcon,
  XCircleIcon,
} from "@heroicons/react/solid";
import LoadingIcon from "~/components/loading-icon";
import { getTokenList } from "~/constants/tokens";
import { TokenType } from "~/type/token";
import { useTranslation } from "react-i18next";
import { useBalance } from "~/hooks/useBalance";
import { formatNumber } from "~/utils/format";
import { getAddressList } from "~/constants/address-list";
import LoginModal from "~/components/login-modal";
import NetworkSwitcher from "~/components/layout/pay/wallet/network-switcher";
import { getTxHash, updateContentDiscountPercentage } from "~/api/order";
import { getTokenBySymbolAndNetwork, getTokensByNetworkId } from "~/api/token";
import { ContractTransaction, ethers } from "ethers";
import { usePoolRateCustomerPay } from "~/hooks/use-pool-rate-customer-pay";
import { getBlockScanner } from "~/constants/block-scanner";
import { toastMessage } from "~/utils/toast";
import ToastMessage from "../toastMessage";
import { LightBulbIcon } from "@heroicons/react/outline";
import { PaymentOutput } from "~/enums/payment-output";
import { Big } from "big.js";

type PropType = {
  order: MerchantOrder;
  orderFetched: MerchantOrder;
  setOrder: Dispatch<SetStateAction<MerchantOrder>>;
  chains: Chain[];
};

export default function PayToken({
  order,
  orderFetched,
  setOrder,
  chains,
}: PropType) {
  const { t } = useTranslation("pay");

  const location = useLocation();

  const [, getSigner] = useSigner();
  const [{ data }] = useNetwork();
  const [{ data: accountData }] = useAccount();
  const [isLoading, setIsLoading] = useState(false);
  const [isProcessing, setIsProcessing] = useState(false);
  const [isPaySubmit, setIsPaySubmit] = useState(false);

  const [isCountDownFinish, setIsCountDownFinish] = useState(false);
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [receiveToken, setReceiveToken] = useState({} as TokenType);
  const [txhashFromEvent, setTxhashFromEvent] = useState("");
  const txForClick = txhashFromEvent || order.transactionHash;
  const url = `${location.pathname}${location.search}`;
  const isUnsupportedNetwork = chains.find(
    (chain) => chain.id === data.chain?.id
  )
    ? false
    : true;

  const addressList = getAddressList(data.chain?.id!);
  const tokenList = getTokenList(data.chain?.id!);
  const defaultToken = tokenList[0] || {};

  const updateDiscountAfterPay = () => {};

  const [payToken, setPayToken] = useState(defaultToken);
  const { showApprove, fetchApproval } = useApproveToken(
    payToken?.tokenAddress!
  );

  const [orderState, setOrderState] = useState<{
    status: OrderState;
    timestamp: string;
  }>({ status: "idle", timestamp: "" });
  useOrderStatus(order, setOrderState, setOrder);
  const [startCountdownRefresh, setStartCountdownRefresh] = useState(false);
  const [countdownRefresh, setCountdownRefresh] = useState(10);
  const isPayPHCP = () => {
    if (payToken?.tokenSymbol === "PHCP") {
      return (Number(order.price) * 9) / 10;
    } else {
      return Number(order.price);
    }
  };

  const tokenPrice = isPayPHCP() / Number(order.exchangeRate);

  const [{ loading }] = useConnect();

  // const [isFetchingAmountIn, amountIn] = useAmountIn(
  //   tokenPrice.toString(),
  //   payToken,
  //   receiveToken
  // );

  // const [isFetchingPoolRate, poolRate] = useAmountIn(
  //   "1",
  //   payToken,
  //   receiveToken
  // );

  const getBNBToken = () => {
    const BNBtoken = tokenList.find((token) => token.tokenSymbol === "BNB");
    if (!BNBtoken) {
      throw new Error("BNB not found");
    }
    return BNBtoken;
  };

  const [
    amountIn,
    poolRate,
    isFetchingAmountIn,
    isFetchingPoolRate,
    amountInPlus,
  ] = usePoolRateCustomerPay({
    payToken,
    receiveToken,
    loading: loading as boolean,
    tokenPrice,
    getBNBToken,
    selectedChain: data.chain as Chain,
  });

  /*
    calculate token rate from pool to exchange rate
      pool rate: 1.00499 DAI/USDC
      exchange rate: 34.48 DAI/THB -> 0.029 THB/DAI
      result: 1.00499 / ( 1 / 34.48) = 34.654 USDC/THB 
  */
  const poolToExchangeRate = formatNumber(
    Number(poolRate) * Number(order.exchangeRate),
    4
  );
  const displayTokenRate = `1 ${payToken.tokenSymbol}: ${
    isFetchingPoolRate || isFetchingAmountIn ? "loading..." : poolToExchangeRate
  } ${order.currency}`;

  const [isFetchingBalance, balance] = useBalance(payToken.tokenSymbol!);

  const isInsufficientBalance = Number(balance || 0) < Number(amountIn || 0);

  const disabled = countdownRefresh !== 0;

  const handlePay = async () => {
    try {
      setIsPaySubmit(true);
      if (amountIn === "0") return null;
      setIsLoading(true);
      const signer = await getSigner();
      const transcrypt = Transcrypt__factory.connect(
        addressList["Transcrypt"],
        signer!
      );

      let tx: ContractTransaction;
      //เหรียญเดียวกันไม่ต้องเผื่อ amountIn
      if (
        payToken?.tokenAddress?.toLowerCase() ===
        receiveToken?.tokenAddress?.toLowerCase()
      ) {
        tx = await transcrypt.purchase(
          Number(order.orderId),
          order.merchant,
          [payToken.tokenAddress!, receiveToken.tokenAddress!],
          parseUnits(amountIn, payToken.tokenDecimal),
          parseUnits(tokenPrice.toString(), receiveToken.tokenDecimal),
          order.deadline,
          "0x0000000000000000000000000000000000000000"
        );
      } else {
        //คนละเหรียญเผื่อ amountIn

        //case phcp ต้องผ่าน bnb
        if (
          payToken?.tokenSymbol === "PHCP" ||
          receiveToken?.tokenSymbol === "PHCP"
        ) {
          const bnbToken = getBNBToken();
          tx = await transcrypt.purchase(
            Number(order.orderId),
            order.merchant,
            [
              payToken.tokenAddress!,
              bnbToken.tokenAddress!,
              receiveToken.tokenAddress!,
            ],
            parseUnits(`${amountInPlus}`, payToken.tokenDecimal),
            parseUnits(tokenPrice.toString(), receiveToken.tokenDecimal),
            order.deadline,
            "0x0000000000000000000000000000000000000000"
          );

          //update content field discount-percentage after pay
          if (payToken.tokenSymbol === "PHCP") {
            await updateContentDiscountPercentage(order.orderId, "10");
          }
        } else {
          tx = await transcrypt.purchase(
            Number(order.orderId),
            order.merchant,
            [payToken.tokenAddress!, receiveToken.tokenAddress!],
            parseUnits(`${amountInPlus}`, payToken.tokenDecimal),
            parseUnits(tokenPrice.toString(), receiveToken.tokenDecimal),
            order.deadline,
            "0x0000000000000000000000000000000000000000"
          );
        }
      }

      const receipt = await tx.wait();
      const event = receipt.events?.find(
        (event) => event.event === "Purchased"
      );

      if (receipt.events && event) {
        const blockTime = await event.getBlock();
        const timestamp = new Date(blockTime.timestamp * 1000).toLocaleString();
        let amountOutBackend;
        let amountOutContract;
        let diff;
        let percent;

        amountOutBackend = new Big(Number(order.amountOut));
        const amountOutASNumber = formatEther(event?.args?.amountOut);
        amountOutContract = new Big(amountOutASNumber);
        diff = amountOutBackend.minus(amountOutContract).abs();

        switch (order.paymentOutput) {
          case PaymentOutput.FIAT:
            if (Number(amountOutBackend) !== 0) {
              percent = Big(diff).div(amountOutBackend).mul(100);
            } else {
              //FIX ME: recheck amountOutBackend later
              percent = 0;
            }
            break;
          case PaymentOutput.TOKEN:
            percent = diff.div(amountOutBackend).mul(100);
            break;
        }

        if (Number(percent) > 10) {
          amountOutBackend = new Big(Number(order.amountOut))
            .mul(Big(9))
            .div(Big(10));
          diff = amountOutBackend.minus(amountOutContract).abs();
          switch (order.paymentOutput) {
            case PaymentOutput.FIAT:
              if (Number(amountOutBackend) !== 0) {
                percent = Big(diff).div(amountOutBackend).mul(100);
              } else {
                //FIX ME: recheck amountOutBackend later
                percent = 0;
              }
              break;
            case PaymentOutput.TOKEN:
              percent = diff.div(amountOutBackend).mul(100);
              break;
          }
        }

        if (Number(percent) > 1) {
          setOrderState({
            status: "failed",
            timestamp,
          });
        } else {
          setOrderState({
            status: "success",
            timestamp,
          });
          setIsProcessing(true);
        }

        const calculatePayAmount = () => {
          if (event?.args?.inputToken === event?.args?.outputToken) {
            return formatNumber(
              formatEther(event?.args?.amountOut.add(event?.args?.fee))
            );
          } else {
            return formatNumber(formatEther(event?.args?.amountIn));
          }
        };

        if (setOrder) {
          setOrder({
            ...order,
            payAmount: calculatePayAmount(),
          });
        }
      }

      const receiptEvent = await getTxHash(event as ethers.Event);

      if (event?.args && receipt?.transactionHash) {
        const blockTime = await event.getBlock();
        const timestamp = new Date(blockTime.timestamp * 1000).toLocaleString();
        if (data.chain?.id) {
          setTxhashFromEvent(
            `${getBlockScanner(data?.chain?.id)}/${receipt.transactionHash}`
          );
        }
      }
      setIsProcessing(false);
      setIsPaySubmit(false);
    } catch (error) {
      setIsPaySubmit(false);
      setIsLoading(false);
      if (error instanceof Error) {
        toastMessage("Something went wrong", error);
      }
    }
  };

  useEffect(() => {
    if (isCountDownFinish && orderState.status !== OrderStatus.SUCCESS) {
      setOrderState({
        status: "expired",
        timestamp: new Date().toLocaleString(),
      });
    }
  }, [isCountDownFinish]);

  useEffect(() => {
    setPayToken(defaultToken);
  }, [defaultToken.tokenAddress]);

  useEffect(() => {
    if (orderState.status === OrderStatus.SUCCESS && order.payToken) {
      const tokenSymbol = tokenList.find(
        (token) => token.tokenSymbol === order.payToken
      );
      if (tokenSymbol) {
        setPayToken(tokenSymbol);
      }
    }
  }, [order.payToken, orderState.status]);

  // Fetch receive token by user pay chain
  useEffect(() => {
    const fetchReceiveToken = async () => {
      if (data.chain?.id) {
        const receiveToken = await getTokenBySymbolAndNetwork(
          order.receiveToken,
          data.chain?.id?.toString()!
        );
        setReceiveToken(receiveToken);
      }
    };
    fetchReceiveToken();
  }, [data.chain?.id]);

  const isPoolNotFound = formatNumber(amountIn) === "NaN";

  useEffect(() => {
    if (startCountdownRefresh && countdownRefresh > 0) {
      setTimeout(() => setCountdownRefresh(countdownRefresh - 1), 1000);
    }
  }, [startCountdownRefresh, countdownRefresh]);

  useEffect(() => {
    if (orderState.status === OrderStatus.SUCCESS) {
      setStartCountdownRefresh(true);
    }
  }, [orderState.status]);

  const OrderPending = () => {
    return (
      <>
        <h1 className="mt-4 mb-10 flex justify-center text-3xl font-extrabold tracking-tight text-gray-900">
          <span className="font-bold text-primary-yellow xl:inline">
            {t("payment")}
          </span>
        </h1>
        <div className="flex items-end justify-between">
          <h1 className="text-2xl font-medium">
            <span className="text-sm font-bold text-white">
              {t("order")} #{order.orderId}
            </span>
          </h1>
          <Countdown
            message="Remaining time"
            deadline={order.deadline}
            isCountDownFinish={isCountDownFinish}
            setIsCountDownFinish={setIsCountDownFinish}
          />
        </div>
        <div className="mt-5 flex justify-between">
          <h1 className="text-sm font-bold text-white">
            {t("merchant_address")}
          </h1>
          <p className="overflow-y-auto text-right text-sm text-white">
            {order.merchantName} ({shorten(order.merchant)})
          </p>
        </div>

        <div className="mt-5 flex justify-between">
          {payToken.tokenSymbol === "PHCP" ? (
            <h1 className="text-sm font-bold text-white">
              {t("total_with_discount")}
            </h1>
          ) : (
            <h1 className="text-sm font-bold text-white">{t("total")}</h1>
          )}
          <p className="text-sm text-white">
            {formatNumber(
              payToken.tokenSymbol === "PHCP"
                ? ((Number(order.price) * 9) / 10).toFixed(2)
                : order.price
            )}{" "}
            {order.currency}
          </p>
        </div>

        {!accountData?.address ? (
          <>
            <LoginModal
              redirectPath={url}
              isOpen={showLoginModal}
              setIsOpen={() => {}}
            />
            <div
              className="mt-3 inline-flex cursor-pointer justify-center rounded-md border border-transparent bg-primary-yellow px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-primary-black-gray focus:outline-none focus:ring-2 focus:ring-primary-yellow focus:ring-offset-2"
              onClick={() => setShowLoginModal(true)}
            >
              {t("connect_wallet")}
            </div>
          </>
        ) : isUnsupportedNetwork ? (
          <>
            <NetworkSwitcher />
          </>
        ) : (
          <>
            <div className="mt-5 flex flex-col justify-between">
              <>
                <h1 className="mb-3 text-sm font-bold text-white">
                  {t("pay_with")}
                </h1>
                <TokenListbox
                  selected={payToken}
                  setSelected={setPayToken}
                  tokens={tokenList.filter(
                    (token) =>
                      token?.tokenSymbol !== "BNB" &&
                      token?.tokenSymbol !== "WIS"
                  )}
                  message={displayTokenRate}
                />
              </>
            </div>

            <div className="mt-5 flex justify-between">
              <h1 className="text-sm font-bold text-white">
                {t("your_balance")}
              </h1>
              <p className=" text-white">
                {isFetchingBalance
                  ? "loading..."
                  : `${formatNumber(balance)} ${
                      payToken.tokenSymbol === undefined
                        ? ""
                        : payToken.tokenSymbol
                    }`}
              </p>
            </div>
            {isPoolNotFound ? (
              <p className="mt-5 text-center text-sm font-bold text-white">
                {t("please_choose_other_tokens")}
              </p>
            ) : (
              <div className="mt-5 flex justify-between">
                <h1 className="text-sm font-bold text-white">
                  {t("payment_amount")}
                </h1>
                <p className="overflow-y-auto text-white">
                  {payToken === receiveToken
                    ? `${formatNumber(amountIn)} ${
                        payToken.tokenSymbol === undefined
                          ? ""
                          : payToken.tokenSymbol
                      }`
                    : `≈ ${formatNumber(amountIn)} - ${formatNumber(
                        amountInPlus
                      )} ${
                        payToken.tokenSymbol === undefined
                          ? ""
                          : payToken.tokenSymbol
                      }`}
                </p>
              </div>
            )}
            {isPoolNotFound ? null : (
              <div className="mt-2 flex justify-center">
                {showApprove ? (
                  <ApproveToken token={payToken} onApprove={fetchApproval} />
                ) : (
                  <button
                    className="inline-flex justify-center rounded-md border border-transparent bg-primary-yellow px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-primary-yellow focus:outline-none focus:ring-2 focus:ring-primary-yellow focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-gray-400"
                    onClick={handlePay}
                    disabled={
                      isCountDownFinish ||
                      showApprove ||
                      amountIn == "0" ||
                      isInsufficientBalance ||
                      isFetchingPoolRate ||
                      isPaySubmit
                    }
                  >
                    {isLoading ? (
                      <>
                        <Loading />
                        {t("processing")}...
                      </>
                    ) : (
                      t("pay")
                    )}
                  </button>
                )}
              </div>
            )}
          </>
        )}
      </>
    );
  };

  const OrderSuccess = () => {
    return (
      <>
        {!isProcessing || txForClick ? (
          <div>
            <h1 className="mb-10 flex justify-center text-3xl font-extrabold tracking-tight text-gray-900">
              <span className="font-bold text-primary-yellow xl:inline">
                {t("payment_successful")}
              </span>
            </h1>
            <div className="flex flex-col items-center justify-center">
              <CheckCircleIcon className="h-32 text-green-400" />
            </div>
            <div className="flex justify-between">
              <h1 className="text-2xl font-medium">
                <span className="text-sm font-bold text-white">
                  {t("order")} #{order.orderId}
                </span>
              </h1>
            </div>
            <div className="mt-5 flex justify-between">
              <h1 className="text-sm font-bold text-white">{t("status")}</h1>
              <p className="text-sm text-primary-yellow">{t("completed")}</p>
            </div>
            <div className="mt-5 flex justify-between">
              <h1 className="text-sm font-bold text-white">
                {t("merchant_address")}
              </h1>
              <p className="overflow-y-auto text-right text-sm text-white">
                {order.merchantName} ({shorten(order.merchant)})
              </p>
            </div>
            <div className="mt-5 flex justify-between">
              <h1 className="text-sm font-bold text-white">{t("total")}</h1>
              <p className="text-sm text-white">
                {formatNumber(
                  payToken.tokenSymbol === "PHCP"
                    ? ((Number(order.price) * 9) / 10).toFixed(2)
                    : order.price
                )}{" "}
                {order.currency}
              </p>
            </div>

            {/* <div className="mt-5 flex justify-between">
          <h1 className="text-sm font-bold text-white">{t("paid_amount")}</h1>
          <p className="overflow-y-auto text-sm text-white">
            {Number(amountIn) !== 0
              ? formatNumber(amountIn)
              : formatNumber(order.payAmount)}{" "}
            {payToken.tokenSymbol}
          </p>
        </div> */}
            {formatNumber(order.payAmount) !== "0" ? (
              <div className="mt-5 flex justify-between">
                <h1 className="text-sm font-bold text-white">
                  {t("paid_amount")}
                </h1>
                <p className="overflow-y-auto text-sm text-white">
                  {formatNumber(order.payAmount)} {payToken.tokenSymbol}
                </p>
              </div>
            ) : null}
            <div className="mt-5 flex justify-center">
              <p className="text-white">{orderState.timestamp}</p>
            </div>
            <div className="mt-5 flex justify-between">
              <button className="h-10 rounded-md border border-primary-yellow bg-white px-6 font-medium text-primary-yellow transition-colors hover:bg-primary-yellow hover:text-white">
                <a href={txForClick} target="_blank">
                  {t("view_on_block_explorer")}
                </a>
              </button>
              <button className="h-10 rounded-md bg-primary-yellow px-6 font-medium text-white transition-colors hover:border hover:border-primary-yellow hover:bg-white hover:text-primary-yellow">
                <Link to="/pay/history">{t("back")}</Link>
              </button>
            </div>
          </div>
        ) : (
          <div>
            <ProcessingTransaction />
          </div>
        )}
      </>
    );
  };

  const ProcessingTransaction = () => {
    return (
      <>
        <div>
          <div className="text-center">
            <h1 className="mb-10 flex justify-center text-3xl font-extrabold tracking-tight text-gray-900">
              <span className="font-bold text-primary-yellow xl:inline">
                {t("processing_transaction")}
              </span>
            </h1>
            <div className="flex h-52 items-center justify-center">
              <div className="">
                <div className="flex justify-center">
                  <LoadingIcon />
                </div>
              </div>
            </div>
          </div>
          <div className="mt-5 flex justify-center text-center text-primary-yellow">
            <div className="mt-5 flex items-center justify-center rounded-md  border-2 border-transparent border-yellow-400 bg-yellow-50 px-3 py-2 text-xs font-medium text-gray-500 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-light-blue focus:ring-offset-2">
              <LightBulbIcon className="mr-2 w-6 text-yellow-400" />
              <p>{t("try_refresh")}</p>
            </div>
          </div>
          <div className="mt-5 flex flex-col items-center justify-center">
            <button
              disabled={disabled}
              onClick={() => {
                window.location.reload();
              }}
              className="
                    inline-flex justify-center rounded-full border border-transparent bg-primary-yellow px-4 py-2 text-sm font-medium text-white 
                    shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-gray-300
                    "
            >
              <p>{t("refresh")}</p>
              {countdownRefresh !== 0 ? (
                <div className="text-center text-sm">({countdownRefresh})</div>
              ) : null}
            </button>
          </div>
        </div>
      </>
    );
  };

  const OrderExpire = () => {
    return (
      <>
        <h1 className="mb-10 flex justify-center text-3xl font-extrabold tracking-tight text-gray-900">
          <span className="font-bold text-primary-yellow xl:inline">
            {t("payment_expired")}
          </span>
        </h1>
        <div className="flex flex-col items-center justify-center">
          <ClockIcon className="h-32 text-red-500" />
        </div>
        <div className="mt-5 flex justify-between">
          <h1 className="text-2xl font-medium">
            <span className="text-sm font-bold text-primary-yellow">
              {t("order")} #{order.orderId}
            </span>
          </h1>
        </div>
        <div className="mt-5 flex justify-between">
          <h1 className="text-sm font-bold text-white">{t("status")}</h1>
          <p className="text-sm text-red-500">{t("expired")}</p>
        </div>
        <div className="mt-5 flex justify-between">
          <h1 className="text-sm font-bold text-white">
            {t("merchant_address")}
          </h1>
          <p className="overflow-y-auto text-right text-sm text-white">
            {order.merchantName} ({shorten(order.merchant)})
          </p>
        </div>
        <div className="mt-5 flex justify-between">
          <h1 className="text-sm font-bold text-white">{t("total")}</h1>
          <p className="text-sm text-white">
            {formatNumber(order.price)} {order.currency}
          </p>
        </div>
        <div className="mt-5 flex justify-center">
          <p className="text-sm text-white">{orderState.timestamp}</p>
        </div>
        <div className="mt-5 flex justify-center">
          <button className="h-10 rounded-md bg-primary-yellow px-6 font-medium text-white transition-colors hover:border hover:border-primary-yellow hover:bg-white hover:text-primary-yellow">
            <Link to="/pay/history">{t("back")}</Link>
          </button>
        </div>
      </>
    );
  };

  const OrderFailed = () => {
    return (
      <>
        <h1 className="mb-10 flex justify-center text-3xl font-extrabold tracking-tight text-gray-900">
          <span className="font-bold text-primary-yellow xl:inline">
            {t("payment_failed")}
          </span>
        </h1>
        <div className="flex flex-col items-center justify-center">
          <XCircleIcon className="h-32 text-red-500" />
        </div>
        <div className="mt-5 flex justify-between">
          <h1 className="text-2xl font-medium">
            <span className="text-sm font-bold text-primary-yellow">
              {t("order")} #{order.orderId}
            </span>
          </h1>
        </div>
        <div className="mt-5 flex justify-between">
          <h1 className="text-sm font-bold text-white">{t("status")}</h1>
          <p className="text-sm text-red-500">{t("failed")}</p>
        </div>
        <div className="mt-5 flex justify-between">
          <h1 className="text-sm font-bold text-white">
            {t("merchant_address")}
          </h1>
          <p className="overflow-y-auto text-right text-sm text-white">
            {order.merchantName} ({shorten(order.merchant)})
          </p>
        </div>
        <div className="mt-5 flex justify-between">
          <h1 className="text-sm font-bold text-white">{t("total")}</h1>
          <p className="text-sm text-white">
            {formatNumber(order.price)} {order.currency}
          </p>
        </div>
        <div className="mt-5 flex justify-center">
          <p className="text-sm text-white">{orderState.timestamp}</p>
        </div>
        <div className="mt-5 flex justify-center">
          <button className="h-10 rounded-md bg-primary-yellow px-6 font-medium text-white transition-colors hover:border hover:border-primary-yellow hover:bg-white hover:text-primary-yellow">
            <Link to="/pay/history">{t("back")}</Link>
          </button>
        </div>
      </>
    );
  };

  return (
    <div className="flex items-center justify-center gap-10">
      <div className="wallet-pay z-20 mt-16 border-primary-black-gray p-8 shadow-md md:w-96">
        <div className="w-80 max-w-md">
          <div className="grid grid-cols-1">
            {orderState.status === OrderStatus.IDLE ? (
              <div className="flex h-52 items-center justify-center">
                <LoadingIcon />
              </div>
            ) : orderState.status === OrderStatus.PENDING ? (
              <OrderPending />
            ) : isProcessing || orderState.status === OrderStatus.SUCCESS ? (
              <OrderSuccess />
            ) : orderState.status === OrderStatus.EXPIRED ? (
              <OrderExpire />
            ) : (
              <OrderFailed />
            )}
          </div>
        </div>
        <ToastMessage />
      </div>
    </div>
  );
}
