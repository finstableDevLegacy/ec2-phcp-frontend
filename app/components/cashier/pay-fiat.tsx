...
import { updateContentDiscountPercentage } from "~/api/order";
...
import { getAmountInWithPoolRate } from "~/hooks/get-amount-in-with-pool-rate";
...
import NetworkSwitcher from "../layout/pay/wallet/network-switcher";
...
export default function PayFiat({ orderFetched, chains }: PropType) {
  ...
  const [fiatExchangeRate, setFiatExchangeRate] = useState<number>();
  ...
  // const [orderIdDealer, serOrderIdDealer] = useState("");
  const [amountInWhenPaying, setAmountInWhenPaying] = useState({
    amountIn: "",
    amountInPlus: "",
  });
  const location = useLocation();
  const callApiGetFiatPurchaseRate = () => {
  ...
  useEffect(() => {
    const fetchFiatRate = async () => {
      if (data?.chain?.id) {
        const rateResponse = await callApiGetFiatPurchaseRate();
        if (order?.receiveToken && rateResponse) {
          setFiatExchangeRate(rateResponse?.price);
          setDealerAddress(rateResponse?.dealerAddress);
        }
      }
    };
    fetchFiatRate();
  }, [data?.chain?.id, order?.receiveToken]);
  ...
  const [poolRate, isFetchingPoolRate, amountIn, amountInPlus] =
    useAmountInWithPoolRate({
      payToken,
      receiveToken,
      selectedChain: data.chain as Chain,
      receiveTokenPricePerFiat: fiatExchangeRate as number,
      totalFiatRequire: +order?.price,
    });
  ...
  const OrderPending = (): JSX.Element => {
    const poolToExchangeRate = formatNumber(
      Number(poolRate) * Number(fiatExchangeRate),
      4
    );
    const displayTokenRate = `1 ${payToken?.tokenSymbol}: ${
      isFetchingPoolRate ? "loading..." : poolToExchangeRate
    } ${order.currency}`;
        ...
        <div className="mt-5 flex justify-between">
          ...
          <p className="overflow-y-auto text-white">
            {payToken === receiveToken
              ? `${formatNumber(
                  isLoading &&
                    amountInWhenPaying?.amountIn &&
                    amountInWhenPaying?.amountIn !== "0"
                    ? amountInWhenPaying.amountIn
                    : amountIn
                )} ${
              ...
              : `≈ ${formatNumber(
                  ...
                  isLoading &&
                    amountInWhenPaying?.amountInPlus &&
                    amountInWhenPaying?.amountInPlus !== "0"
                    ? amountInWhenPaying.amountInPlus
                    : amountInPlus
                )} ${
                  payToken?.tokenSymbol === undefined
    ...
    const handlePay = async () => {
      ...
      const expectedReceiveTokenAmount = tokenPrice / Number(fiatExchangeRate);
      if (!data?.chain?.id) {
        return;
      }
      try {
        setIsLoading(true);
        const fiatRateBeforePay = await callApiGetFiatPurchaseRate();
        ...
        const [poolRateRefetched, amountInRefetched, amountInPlusRefetched] =
          await getAmountInWithPoolRate({
            payToken,
            receiveToken,
            selectedChain: data.chain as Chain,
            receiveTokenPricePerFiat: fiatRateBeforePay?.price as number,
            totalFiatRequire: +order?.price,
          });
        setAmountInWhenPaying({
          amountIn: amountInRefetched,
          amountInPlus: amountInPlusRefetched,
        });
        if (amountInRefetched === "0") return null;
        ...
        if (payToken === receiveToken) {
          tx = await transcrypt.purchase(
            ...
            parseUnits(amountInRefetched, payToken.tokenDecimal),
            parseUnits(
              expectedReceiveTokenAmount.toString(),
              receiveToken.tokenDecimal
            ),
            order.deadline,
            dealerAddress
          );
          ...
          if (
            ...
            tx = await transcrypt.purchase(
              ...
              parseUnits(`${amountInPlusRefetched}`, payToken.tokenDecimal),
              parseUnits(
                expectedReceiveTokenAmount.toString(),
                receiveToken.tokenDecimal
              ),
        ...
        if (event?.args) {
          ...
          // const { id: dealerOrderId } = await apiCreateMatchOrder(
          //   receipt.transactionHash,
          //   orderIdDealer,
          //   data.chain.id
          // );
          // await apiUpdateDealerOrderId(order.orderId, dealerOrderId);
          setOrder({
            ...order,
    ...
    const renderPayButton = (): JSX.Element => {
      const isInsufficientBalance =
        Number(balance || 0) < Number(amountIn || 0);
      if (isPoolNotFound) {
        return <></>;
      }
      return (
      ...
      <>
        ...
        {!accountData?.address ? (
          <>
            ...
            />
            <div
              className="hover:bg-ssecondary-dark-blue mt-3 inline-flex cursor-pointer justify-center rounded-md border border-transparent bg-primary-yellow px-4 py-2 text-sm font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-secondary-dark-blue focus:ring-offset-2"
  ...
  const OrderExpire = () => {
    return (
      <>
        <h1 className="mb-10 flex justify-center text-3xl font-extrabold tracking-tight text-white">
          <span className="font-bold text-primary-yellow xl:inline">
            {t("payment_expired")}
          </span>
        </h1>
  ...
  const renderByStatus = (): JSX.Element => {
    ...
    if (orderState.status === OrderStatus.PENDING || !order?.transactionHash) {
      return <OrderPending />;
    }
